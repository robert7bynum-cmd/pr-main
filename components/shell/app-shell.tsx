...
export function AppShell({ user, children }: { user: ShellUser; children: React.ReactNode }) {
  ...
  return (
    <div className="app-ground min-h-dvh text-ink antialiased">
      <header className="sticky top-0 z-30 border-b border-line bg-surface-app/85 backdrop-blur-md">
        <div className="mx-auto flex max-w-[62rem] items-center justify-between gap-3 px-5 py-3.5">
          <button
            onClick={() => setOpen(true)}
            aria-label="Menu"
            className="flex h-10 w-10 items-center justify-center rounded-control border border-line bg-surface-raised shadow-card transition hover:border-line-strong"
          >
          ...
          <p className="min-w-0 truncate font-display text-[14px] tracking-tight text-ink-secondary">
            {user.course_name}
          </p>
          ...
          <div className="relative">
            <button
              onClick={() => setMenu((m) => !m)}
              className="flex items-center gap-2 rounded-control border border-line bg-surface-raised py-1.5 pl-1.5 pr-2.5 shadow-card transition hover:border-line-strong"
            >
              {/* Initials rather than an avatar: nobody uploads a photo to a
                  work tool, and a broken image is worse than none. */}
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-accent-strong text-[11px] font-semibold text-ink-on-accent">
            ...
            {menu && (
              <>
                ...
                <div className="absolute right-0 z-20 mt-2 w-60 rounded-card border border-line bg-surface-raised py-2 shadow-pop">
      ...
      >
        ...
        />
        <nav
          className={`absolute inset-y-0 left-0 w-[18rem] border-r border-line bg-surface-raised shadow-pop
        ...
        >
          <div className="border-b border-line px-6 pt-7 pb-5">
            ...
            <p className="mt-2 font-display text-[19px] leading-tight tracking-tight">
              {user.course_name}
            </p>
            ...
            {items.map((n) => {
              ...
              return (
                <li key={n.href}>
                  <a
                    href={n.href}
                    className={`block rounded-control px-4 py-3 transition ${
                      active
                        ? "border border-accent-border bg-accent-surface"
